...
class Stream(object):
    ...
    def download(self, filepath="", quiet=False, callback=lambda *x: None,
        ...
        if filepath and os.path.isdir(filepath):
            savedir, filename = filepath, self._title#self.generate_filename()

        elif filepath:
            savedir, filename = os.path.split(filepath)

        else:
            filename = self._title#self.generate_filename(meta=meta)

        filepath = os.path.join(savedir, filename)
        temp_filepath = filepath + ".temp"

        status_string = ('  [{:.2%}] received.')
        ...
        while self._active:
            ...
            progress_stats = ( bytesdone * 1.0 / total)
